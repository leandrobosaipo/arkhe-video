...
- Parâmetros de requisição com exemplos
- Schemas de resposta
- Exemplos de uso
...
- **Requirement**: Optional if using DigitalOcean Spaces (will be extracted from endpoint URL). Mandatory for other S3-compatible services. "None" is acceptable for some S3 providers.

### DigitalOcean Spaces Configuration

DigitalOcean Spaces is fully supported and recommended for this API. The system automatically detects DigitalOcean Spaces endpoints and can extract bucket name and region from the endpoint URL if not explicitly provided.

**To configure DigitalOcean Spaces:**

1. Create a Space in your DigitalOcean dashboard
2. Generate Access Keys (API → Spaces Keys)
3. Set the following environment variables:
   - `S3_ENDPOINT_URL`: Your Space endpoint (e.g., `https://your-bucket.nyc3.digitaloceanspaces.com`)
   - `S3_ACCESS_KEY`: Your Spaces Access Key
   - `S3_SECRET_KEY`: Your Spaces Secret Key
   - `S3_BUCKET_NAME`: (Optional) Will be extracted from URL if not provided
   - `S3_REGION`: (Optional) Will be extracted from URL if not provided

See [ENV_EXAMPLE.md](ENV_EXAMPLE.md) for detailed configuration instructions.
...
EasyPanel é uma plataforma simples e eficiente para fazer deploy de aplicações Docker.

- **[Guia de Deploy no EasyPanel](DEPLOY_EASYPANEL.md)** - Instruções completas para deploy no EasyPanel
